...
const ProductSchema = new Schema(
  {
    ...
    /**
     * Отпечаток последней применённой выгрузки 1С.
     *
     * 1С шлёт полный каталог каждый обмен (десятки раз в сутки), а меняются в
     * нём единицы позиций. Совпал отпечаток — карточку не трогаем вовсе,
     * обновляя только метку `product1cSeenAt` одним запросом на пачку.
     */
    ...
    /**
     * Карточка снята с витрины правилом обмена «нет картинок и нет остатка».
     *
     * Раньше такую карточку удаляли, а при возврате остатка создавали заново —
     * вместе с новым `_id`, потерянными отзывами, избранным и, главное, новой
     * очередью модерации на весь каталог. Теперь она просто прячется и ждёт
     * ближайшего `rests.xml`.
     */
    product1cHeld: {
      type: Boolean,
      default: false,
    ...
    /**
     * Отпечаток того содержимого, которое одобрил модератор: название,
     * описание, фото, категория, характеристики. Цены и остатка в нём нет — их
     * изменение к повторной проверке отношения не имеет.
     *
     * Считается в момент одобрения. По нему карточка, которую пришлось завести
     * заново (вернулась из отстойника обмена), получает прежний статус, если
     * содержимое не изменилось, — вместо похода в очередь по второму кругу.
     */
