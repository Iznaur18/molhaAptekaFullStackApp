...
const OrderShipmentSchema = new mongoose.Schema(
  {
     ...
     *
     * Снимок, а не чтение вживую: ставку можно поменять, но к уже
     * оформленным заказам это отношения иметь не должно — продавец видел
     * одно, а получил бы другое.
     */
    platformCommissionPercentAtOrder: { type: Number, default: null, min: 0 },
    /**
    ...
    /**
     * Доставка СДЭК до пункта выдачи — снимок на момент заказа: тариф, цена,
     * срок и сам пункт. Цена информационная: покупатель платит её СДЭК при
     * получении, в totalAmount она не входит (решение 21.09.2026).
     */
    cdekShipmentAtOrder: {
      type: mongoose.Schema.Types.Mixed,
      default: undefined,
    ...
    /**
     * Накладная СДЭК, созданная продавцом: uuid заказа в СДЭК, трек-номер
     * (cdek_number приходит не сразу — СДЭК обрабатывает заказ асинхронно),
     * пункт приёма и последняя ошибка от службы.
     */
    cdekWaybill: {
      type: mongoose.Schema.Types.Mixed,
      default: undefined,
