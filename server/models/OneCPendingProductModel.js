...
const OneCPendingProductSchema = new mongoose.Schema(
  {
    ...
    /**
     * Статус модерации карточки, которая когда-то существовала под этим `Ид`,
     * и отпечаток одобренного содержимого.
     *
     * Живые карточки правило приёмки больше не удаляет — только прячет, — но
     * записи, оставшиеся от прежнего поведения (и от случая «удалить нельзя,
     * есть незакрытые заказы»), продолжают приезжать сюда. Если содержимое с
     * тех пор не изменилось, восстановленная карточка получает прежний статус
     * и не идёт в очередь модерации повторно.
     */
    moderationStatus: {
      type: String,
      default: "",
