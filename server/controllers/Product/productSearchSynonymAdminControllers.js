...
const normalizeCategoriesInput = (raw) => {
  if (!Array.isArray(raw)) {
    throw new Error("categories должен быть массивом slug");
  }
  const unique = [
    ...new Set(
      raw
  ...
  if (!unique.length) {
    throw new Error("Укажите хотя бы одну категорию");
  }
...
export async function listProductSearchSynonymsAdminController(_req, res) {
  try {
    ...
    return errorRes(
      res,
      500,
      error instanceof Error ? error.message : "Не удалось загрузить синонимы",
    );
  }
}
...
export async function createProductSearchSynonymAdminController(req, res) {
  try {
    ...
    return errorRes(
      res,
      400,
      error instanceof Error ? error.message : "Не удалось создать синоним",
    );
  }
}
...
export async function patchProductSearchSynonymAdminController(req, res) {
  try {
    ...
    return errorRes(
      res,
      400,
      error instanceof Error ? error.message : "Не удалось обновить синоним",
    );
  }
}
...
export async function deleteProductSearchSynonymAdminController(req, res) {
  try {
    ...
    return errorRes(
      res,
      500,
      error instanceof Error ? error.message : "Не удалось удалить синоним",
    );
  }
}
