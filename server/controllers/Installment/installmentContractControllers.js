...
export const createInstallmentContractController = async (req, res) => {
  try {
    ...
    try {
      ...
      return errorRes(res, 403, e instanceof Error ? e.message : "Нет прав");
    }
    ...
    try {
      ...
      return errorRes(res, 400, e instanceof Error ? e.message : "Недостаточно товара");
    }
...
export const rejectInstallmentEarlyPayoffController = async (req, res) => {
  try {
    ...
    if (!isEarlyPayoffPendingConfirmation(contract)) {
      return errorRes(res, 409, "Нет досрочного погашения для отклонения");
    }
...
export const cancelInstallmentEarlyPayoffController = async (req, res) => {
  try {
    ...
    if (!isEarlyPayoffPendingConfirmation(contract)) {
      return errorRes(res, 409, "Нет досрочного погашения для отмены");
    }
