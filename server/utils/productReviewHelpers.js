...
export const recalculateProductReviewAggregates = async (productId) => {
    ...
    const averageRating =
        reviewCount > 0
            ? Math.round((Number(rows[0].averageRating) || 0) * 10) / 10
            : 0;
...
export const assertCanSubmitProductReview = async (authorUserId, productId) => {
    ...
    const hasConfirmed = await userHasConfirmedProductPurchase(
        authorUserId,
        productId,
    );
    if (!hasConfirmed) {
...
export const buildProductReviewSummary = async (viewerUserId, productId) => {
    const product = await ProductModel.findById(productId)
        .select(
            'averageRating reviewCount productModerationStatus productSeller',
        )
        .lean();
