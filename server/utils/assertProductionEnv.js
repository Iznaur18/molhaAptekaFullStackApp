...
export const assertProductionEnv = () => {
    ...
    if (isBlank(process.env.JWT_SECRET)) {
        ...
        if (PLACEHOLDER_JWT_SECRETS.has(secret)) {
            errors.push('JWT_SECRET — placeholder из .env.example, замените на случайную строку');
        }
    }
    ...
    if (isProduction) {
        if (isBlank(process.env.FRONTEND_URL)) {
            errors.push('FRONTEND_URL обязателен при NODE_ENV=production (CORS + verify email)');
        } else {
    ...
    if (smtpPartial && !smtpComplete) {
        warnings.push('SMTP задан частично — нужны SMTP_HOST, SMTP_USER, SMTP_PASS (и SMTP_FROM)');
    }
