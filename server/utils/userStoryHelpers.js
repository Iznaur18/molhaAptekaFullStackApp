...
import { canModerateProductsRole } from './productModeration.js';
import { deleteUploadFileByUrl } from './deleteUploadFileByUrl.js';
...
export async function expireStaleUserStories(now = new Date()) {
    ...
    await Promise.all(
        staleStories.map((story) => deleteUploadFileByUrl(story.mediaUrl)),
    );
...
export async function createUserStoryRecord({
    ...
    const story = await UserStoryModel.create({
        ...
        captionText: String(captionText ?? '').trim().slice(0, USER_STORY_CAPTION_MAX_CHARS),
        status: USER_STORY_STATUS_ACTIVE,
        publishedAt: now,
...
export async function getUserStoriesFeed(viewerUserId) {
    ...
    const rings = authorIds
        .map((authorId) => {
            ...
            const lastViewedPublishedAt =
                lastViewedPublishedAtByAuthor.get(authorId) ?? null;
            const isViewed =
                isOwn ||
...
export async function getPendingUserStoryReportGroups() {
    ...
    const authorIds = [
        ...new Set(stories.map((story) => String(story.authorUserId))),
    ];
    ...
    const reporterIds = [
        ...new Set(
            rows.flatMap((row) =>
                row.reports.map((report) => String(report.reporterUserId)),
            ),
        ),
    ];
    ...
    const groups = rows
        .map((row) => {
            ...
            const author =
                authorsById[String(story.authorUserId)] ?? {
