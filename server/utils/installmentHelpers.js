...
export const normalizeInstallmentPlanInput = (raw) => {
    ...
    if (
        ...
        throw new Error(
            `Минимальный платёж: ${INSTALLMENT_MONTHLY_PAYMENT_MIN_RUB} ₽`,
        );
    }
 ...
 */
export const rejectInstallmentPaymentPendingConfirmation = (
    contract,
    paymentIndex,
) => {
...
export const canBuyerMarkInstallmentPayment = (contract, payment) => {
    ...
    return (
        nextPayment != null &&
        nextPayment.paymentIndex === payment.paymentIndex
    );
};
...
export const recomputeContractOverdueFlags = (contract) => {
    ...
    for (const payment of contract.payments ?? []) {
        ...
        const isPastDue =
            Boolean(payment.dueAt) && new Date(payment.dueAt) < now;
 ...
 * @param {Date} paidAt
 */
export const applyConfirmedInstallmentPayment = (
    contract,
    paymentIndex,
    paidAt,
) => {
    const payment = contract.payments.find(
        (row) => row.paymentIndex === paymentIndex,
    );
    if (!payment) {
        throw new Error('Платёж не найден');
    }
    ...
    if (nextDue && !contract.payments.some((row) => row.status === INSTALLMENT_PAYMENT_STATUS_DUE)) {
...
export const loadInstallmentCounterpartyMap = async (userIds) => {
    ...
    return new Map(
        rows.map((row) => [
            String(row._id),
            toInstallmentCounterpartyPayload(row),
        ]),
    );
};
 ...
 */
export const attachCounterpartiesToInstallmentContractPayload = (
    payload,
    userMap,
) => ({
    ...payload,
...
export const buildInstallmentContractPayload = async (contract) => {
    const plain =
        typeof contract.toObject === 'function'
            ? contract.toObject()
            : contract;
