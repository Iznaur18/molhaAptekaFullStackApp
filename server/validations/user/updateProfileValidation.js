...
export const updateProfileValidation = [
  ...
  body("userBirthDate")
    ...
    .custom((value) => {
      if (value === null || value === "") {
        // если значение null или пустое, то разрешаем
        return true; // Разрешаем null для очистки
      }
      const date = new Date(value);
      if (isNaN(date.getTime())) {
        // если значение не является валидной датой, то ошибка
        throw new Error("Дата рождения должна быть в формате ISO 8601");
      }
      if (date > new Date()) {
        // если дата в будущем, то ошибка
  ...
  body("userBackgroundUrl")
    ...
    .custom((value) => {
      ...
      throw new Error("Фон: пресет preset:<id> или URL (http/https)");
    }),
