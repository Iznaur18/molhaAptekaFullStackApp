...
export const upsertProductInstallmentProgramValidation = [
  body("isEnabled").isBoolean().withMessage("isEnabled должен быть boolean"),
  body("plans")
  ...
  body("plans.*.monthlyAmountRub")
    ...
    .toInt(),
  body("plans.*.firstPaymentRequiredNow").optional().isBoolean(),
  handleValidationByExpressErrors,
];

export const rejectInstallmentModerationValidation = [
  body("moderationComment").optional().isString().trim().isLength({ max: 2000 }),
  handleValidationByExpressErrors,
];
...
export const resolveInstallmentDisputeValidation = [
  ...
  body("partialRefundRub").optional().isInt({ min: 1 }).toInt(),
  handleValidationByExpressErrors,
];
