 ...
 * открыть devtools.
 *
 * **`tariffGeo` для тарифа больше не используется.** Расстояние доставки
 * продавца теперь считается по дорогам в `resolveDeliveryPoints`: точка
 * ищется по тексту адреса (DaData, OpenStreetMap), а клиентской верят только
 * в пределах погрешности найденной. Правило осталось прежним — расстояние по
 * числу, которое назвал плательщик, не считаем: прислав точку у склада
 * продавца, он обнулил бы километраж.
 *
