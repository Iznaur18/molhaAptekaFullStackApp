...
export async function getFollowingUserIdsForFollower(followerId) {
  const rows = await UserFollowModel.find({ followerId }).select("followingId").lean();
...
export async function getVisibleFollowingSellerIds(followerId) {
  ...
  const hiddenSellerIds = new Set((await getHiddenSellerIds()).map((id) => String(id)));
...
export async function followUser(followerId, followingId) {
  ...
  const follower = await UserModel.findById(followerId).select("userName").lean();
...
export async function listUserFollowRelations({
  ...
  const filter =
    role === "followers" ? { followingId: userId } : { followerId: userId };
  ...
  const [rows, total] = await Promise.all([
    UserFollowModel.find(filter)
      ...
      .skip(skip)
      .limit(safeLimit)
      .populate(role === "followers" ? "followerId" : "followingId", USER_LIST_SELECT)
      .lean(),
