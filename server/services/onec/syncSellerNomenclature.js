...
export async function syncSellerNomenclature(sellerId, opts = {}) {
  ...
  for (const item of items) {
    ...
    // То же правило приёмки, что и в CommerceML: без картинок товар на сайт
    // не заводится. Здесь остаток и картинки приезжают вместе, отстойник не
    // нужен — следующая синхронизация с фото принесёт товар целиком.
    if (!hasImages) {
      if (existing) {
