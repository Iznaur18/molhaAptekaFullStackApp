...
| POST  | `/upload/video`               | `video` | 5 МБ (mp4/webm/mov)  |
| POST  | `/upload/video?purpose=intro` | `video` | 100 МБ — исходник intro; сервер обрезает до 10 с, ограничивает битрейт и хранит только пережатый MP4 (~6-7 МБ), оригинал удаляется |
...
- если задан **`PUBLIC_UPLOAD_BASE_URL`** → полный URL: `https://gitorg.ru/uploads/xxx.mp4`
...
| **Вариант A:** один домен + nginx                             | `https://gitorg.ru/uploads/...` | nginx → Express ✅       |
...
```
https://gitorg.ru/           → nginx → client/dist (статика SPA)
https://gitorg.ru/auth/...   → nginx → proxy → Node :4444
https://gitorg.ru/uploads/   → nginx → proxy → Node
```
...
VITE_API_URL=https://api.gitorg.ru

# server/.env
FRONTEND_URL=https://app.gitorg.ru
PUBLIC_UPLOAD_BASE_URL=https://api.gitorg.ru
```
...
SSL: `certbot --nginx -d gitorg.ru -d www.gitorg.ru`
