...
| Продавец, обычные заказы                                                    | Паспорт **не** отдаётся.                                                                          |
| Продавец, заказ/контракт **рассрочки** (после согласия покупателя)          | Snapshot на `Order.buyerPassportShare`: маскированные серия/номер + selfie (как staff). После отмены заказа snapshot очищается; в API `buyerPassportShare: null`. Audit: `passportShareConsentAt` без значений PII. |
