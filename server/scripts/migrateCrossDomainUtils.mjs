...
const CLUSTERS = {
  order: ["orderLineItemDisplay.js", "sellerTotalSalesAmount.js"],
  loyalty: ["rubBalanceSpend.js"],
  user: ["profileImageFocus.js", "maxWordsText.js"],
  product: ["ruCityNormalized.js"],
};
...
const walkAndUpdateImports = (dir) => {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.name.startsWith("migrate") && entry.name.endsWith(".mjs")) {
      continue;
    }
...
console.log(
  `Migrated ${migratedCount} utils → order/loyalty/user/product; imports updated`,
);
