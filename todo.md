...
1. Сделать обязательный проигрыватель интро ролика в мобильном приложении при первом запуске
2. Продолжить в клауд код "сделай тоже самое для "Мои желания" "Подписки" "Реклама"
3. Поднять до 50мб размеры файлов, которые можно установить в проекте где бы то нибыло. Но важно чтобы они автоматически сжимались и не попадали на серверм с таким большим размером.
4. Изменить поведение рассрочки. Месяц определяется по другому, давай обсудим это.
5. Кнопки ">" поменять на текст "подробнее"
6. Поменять стиль окна Розыгрыш
...
- [x] В веб-версии — мобильный размер сайта (адаптив / mobile viewport)
