...
export const validateAppIntroAdminForm = (form: AppIntroAdminForm): string | null => {
  ...
  if (!Number.isFinite(minMs) || minMs < APP_INTRO_MIN_MS_MIN || minMs > APP_INTRO_MIN_MS_MAX) {
    return APP_INTRO_ADMIN_PAGE_UI.ERROR_MIN_MS;
  }
  if (!Number.isFinite(maxMs) || maxMs < APP_INTRO_MAX_MS_MIN || maxMs > APP_INTRO_MAX_MS_MAX) {
