...
export const campaignModerationIsStale = (
  campaign: CampaignRecord,
  nowMs = Date.now(),
) => {
  const createdAt = campaign.createdAt
    ? new Date(String(campaign.createdAt)).getTime()
    : NaN;
  return (
    Number.isFinite(createdAt) &&
    nowMs - createdAt >= CAMPAIGN_MODERATION_STALE_THRESHOLD_MS
  );
};

export const campaignModerationNeedsAttention = (
  campaign: CampaignRecord,
  nowMs = Date.now(),
) => {
