...
const hook = readMobileFile(
  "features/create-product/model/useCreateProductFormDraft.ts",
);
...
test("черновик хранится синхронно, без SecureStore", () => {
  ...
  const secureStore = readRepoFile(
...
test("автосейв закрывает то, чего в вебе не бывает: ОС убивает приложение молча", () => {
  assert.ok(
    ...
    "нет сохранения при сворачивании",
  );
  ...
  assert.ok(
    hook.includes("subscription.remove()"),
    "подписка на AppState не снимается",
  );
  assert.ok(
    /useEffect\(\(\) => \(\) => flush\(\)/.test(hook),
    "нет флаша при уходе с экрана",
  );
});
...
test("черновик не мешает копированию, правке и отправке", () => {
  ...
  assert.ok(
    screen.includes("CREATE_PRODUCT_UI.DRAFT_RESTORED_HINT"),
    "нет баннера восстановления",
  );
  assert.ok(screen.includes("onPress={discardDraft}"), "нет кнопки «начать заново»");
  assert.ok(
    screen.includes("onPress={handleSaveDraftAndExit}"),
    "нет кнопки «сохранить и выйти»",
  );
});
