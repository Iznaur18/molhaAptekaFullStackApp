...
export function ProductPickupLocationFields({
  ...
  useEffect(() => {
    ...
    // Книга адресов приезжает из сессии и легко опаздывает к первому рендеру:
    // настройки продавца грузятся своим запросом и приходят раньше. Решать без
    // неё нельзя — «ни одного профильного адреса» и «адреса ещё не пришли»
    // выглядят одинаково, а решение защёлкивается навсегда. Из-за этого после
    // перезагрузки страницы сохранённая точка оставалась без галочки, и первое
    // же изменение в блоке вычищало её из формы: prune оставляет только то, что
    // отмечено, а отмеченного не было.
    if (list.length > 0 && profileAddresses.length === 0) {
      return;
    }
