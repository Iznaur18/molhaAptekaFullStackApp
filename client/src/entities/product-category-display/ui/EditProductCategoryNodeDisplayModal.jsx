 ...
 * }} props
 */
export function EditProductCategoryNodeDisplayModal({
  isOpen,
  categoryId,
  fallbackLabel,
  displays,
  ...
  useEffect(() => {
    ...
    if (!didOpen || !categoryId || !fallbackLabel) {
      return undefined;
    }

    const resolvedItem = resolveProductCategoryNodeDisplay(
      categoryId,
      fallbackLabel,
      mapCategoryDisplaysById(displays),
    );
    setLabel(resolvedItem.isCustomLabel ? resolvedItem.label : "");
    setImageUrl(resolvedItem.imageUrl ?? "");
    setErrorMessage("");
    return undefined;
  }, [isOpen, categoryId, fallbackLabel, displays]);
  ...
  const resolved =
    categoryId != null && fallbackLabel != null
  ...
  if (!isOpen || !categoryId || !fallbackLabel || !resolved) {
    return null;
  }
  ...
  return createPortal(
    <div className="edit-category-display-modal__backdrop" role="presentation">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="edit-category-node-display-modal-title"
      >
        <div className="edit-category-display-modal__header">
          <h2
            id="edit-category-node-display-modal-title"
            className="edit-category-display-modal__title"
          >
            {PRODUCT_CATEGORY_DISPLAY_UI.SUBCATEGORY_NODE_EDIT_TITLE(resolved.label)}
          </h2>
          <button
            type="button"
        ...
        <form className="edit-category-display-modal__form" onSubmit={handleSubmit}>
          <label className="edit-category-display-modal__field">
            ...
            <input
              ...
              placeholder={PRODUCT_CATEGORY_DISPLAY_UI.LABEL_PLACEHOLDER(fallbackLabel)}
              maxLength={120}
            />
            <span className="edit-category-display-modal__hint">
              {PRODUCT_CATEGORY_DISPLAY_UI.SUBCATEGORY_NODE_LABEL_HINT}
            </span>
          </label>
