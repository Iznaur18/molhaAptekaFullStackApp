...
export const INTRO_AD_PAGE_UI = {
  ...
  SUBMIT_SUCCESS: "Заявка отправлена на модерацию. Счёт придёт после одобрения.",
  CANCEL_FALLBACK: "Не удалось отменить заявку",
  CANCEL_SUCCESS: "Заявка отменена",
  /** @param {number} price */
  PRICE: (price) => `Стоимость: ${price} ₽`,
  DURATION: "Срок показа: 3 дня",
  DESCRIPTION:
    "Ваш intro-ролик заменит заставку сайта для новых посетителей и при просмотре intro. Сначала модерация, потом счёт: за отклонённую заявку вы не платите.",
  /** @param {number} balance */
  BALANCE: (balance) => `Доступно: ${balance} ₽`,
  STATUS_PENDING: "На модерации. Деньги пока не списываются.",
  STATUS_AWAITING_PAYMENT: "Одобрено. Показ начнётся после оплаты.",
  STATUS_QUEUED: "Оплачено. Ожидает свободного слота.",
  PAY: "Оплатить по СБП",
  PAY_PENDING: "Открываем оплату…",
  PAY_FALLBACK: "Не удалось начать оплату",
  STATUS_ACTIVE: "Показ активен.",
