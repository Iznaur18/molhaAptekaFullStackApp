...
import { INTRO_AD_MODERATION_SECTION_RAFFLE } from "../lib/introAdModerationSectionFilters.js";
...
import "../../../pages/raffles-staff/ui/RafflesStaffPage.css";
 ...
 * }} props
 */
export function RaffleModerationSection({
  onQueueChanged,
  onEditRaffle,
  actionError = "",
  onActionError,
  ...
  const handleApprove = async (raffleId) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : API_CLIENT_UI.APPROVE_RAFFLE_FALLBACK;
  ...
  const handleReject = async (raffleId) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : API_CLIENT_UI.REJECT_RAFFLE_FALLBACK;
      onActionError?.(message);
      setRowErrors((prev) => ({ ...prev, [raffleId]: message }));
    } finally {
      setPendingId(null);
  ...
  const handleDelete = async (raffleId, { clearLive = false } = {}) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : API_CLIENT_UI.DELETE_RAFFLE_FALLBACK;
      onActionError?.(message);
      setRowErrors((prev) => ({ ...prev, [raffleId]: message }));
    } finally {
      setPendingId(null);
  ...
  if (queueQuery.isPending && raffles.length === 0 && !liveRaffleFromQuery) {
    return null;
  }

  if (raffles.length === 0 && !liveRaffle) {
    return null;
  }
  ...
  return (
    <section className="intro-ad-moderation-page__section raffles-staff-page">
      {actionError ? (
        <p
      ...
      {liveRaffle ? (
        <div className="raffles-staff-page__live">
      ...
      {raffles.length > 0 ? (
        <>
          <ModerationSectionTitle
            ...
            pendingCount={raffles.length}
          />
          <ul
            className={`raffles-staff-page__list ${buildIntroAdModerationZonePanelClass(INTRO_AD_MODERATION_SECTION_RAFFLE)}`}
          >
            {raffles.map((raffle) => {
